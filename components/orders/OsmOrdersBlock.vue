<template>
    <div class="orders__block">
        <div class="orders__block--l-side">
            <div class="orders__block--order-number"><span>№ заказа:</span> {{order.id}}</div>
            ...
            <div 
                ...
                <div class="orders__block--item">
                    <div class="orders__block--text">
                        ...
                        <div class="orders__block--description">{{ product.description }}</div>
        ...
        <div class="orders__block--r-side">
            <div class="orders__block--price">
                ...
                <div class="orders__block--more" :class="{'isActive': isMobileShowMore}" @click="onToggleIsMobileShowMore">{{ !isMobileShowMore ? 'Подробнее' : 'Свернуть'}}</div>
            </div>
        </div>
    </div>
</template>
    ...
    computed: {
        ...
        products() {
            ...
            arrProductsBasket.forEach(product => {
                const findObjProduct = this.$store.state.orders.products.find(item => item.id === product.id);
                if (findObjProduct) {
        ...
        resultPrice() {
            ...
            let resultPrice = 0;
            arrProductsBasket.forEach(product => {
                const findProduct = this.$store.state.orders.products.find(item => item.id === product.id);
                if (findProduct) {
